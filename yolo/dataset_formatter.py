...
def process_dataset_split(source_dir, target_images_dir, target_labels_dir, split_name):
        ...
        try:
            ...
            if img_width is None or img_height is None or not objects:
                if img_width is None:
                    print(
                        f"Warning: Could not get image dimensions from {xml_file.name}"
                    )
                else:
    ...
    print(
        f"{split_name} split: {processed_count} files processed, {skipped_count} files skipped"
    )
...
def main():
    ...
    process_dataset_split(
        str(source_train_dir), str(train_images_dir), str(train_labels_dir), "Train"
    )

    # Process validation split
    process_dataset_split(
        str(source_val_dir), str(val_images_dir), str(val_labels_dir), "Validation"
    )
    ...
    print(
        f"Total: {train_images_count + val_images_count} images, {train_labels_count + val_labels_count} labels"
    )
