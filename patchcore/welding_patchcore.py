...
class WeldingPatchCore:
    ...
    def __init__(self, train_data_dir, test_data_dir=None, device=None):
        ...
        # Transform for variable-sized input (no resize - patches already processed)
    ...
    def build_memory_bank(self, subsample_ratio=0.001):  # Much smaller default
        """
        ...
        # Load good welding patches (limit to first 800 images)
        good_dataset = WeldingDataset(
            self.train_data_dir, transform=self.transform, class_type="good"
        )

        # Limit to first 300 images
        if len(good_dataset) > 300:
            print(f"Limiting training set from {len(good_dataset)} to 300 images")
            good_dataset.image_paths = good_dataset.image_paths[:300]

        dataloader = DataLoader(good_dataset, batch_size=1, shuffle=False)
        ...
        max_samples = 5000  # Hard limit to prevent OOM
        if n_samples < 100:  # Ensure minimum samples
            n_samples = min(100, all_features.shape[0])
        elif n_samples > max_samples:  # Ensure maximum samples
            n_samples = max_samples
            print(f"Capping memory bank to {max_samples} samples to prevent OOM")
    ...
    def compute_threshold(self):
        ...
        # Limit to first 300 images (same as memory bank)
        if len(good_dataset) > 300:
            print(
                f"Limiting threshold computation from {len(good_dataset)} to 300 images"
            )
            good_dataset.image_paths = good_dataset.image_paths[:300]

        print(f"Using {len(good_dataset)} images for threshold computation")
        dataloader = DataLoader(good_dataset, batch_size=1, shuffle=False)
        scores = []
    ...
    def predict(self, data_dir):
        ...
        for class_type in ["good", "bad"]:
            ...
            dataset = WeldingDataset(
                data_dir, transform=self.transform, class_type=class_type
            )

            # Limit test set to prevent long processing times
            # if len(dataset) > 1000:
            #     print(f"Limiting {class_type} test set from {len(dataset)} to 1000 images")
            #     dataset.image_paths = dataset.image_paths[:1000]
            ...
            for batch in tqdm(dataloader, desc=f"Predicting {class_type}"):
                ...
                with torch.no_grad():
                    ...
                    distances = torch.cdist(features, self.memory_bank)
                    min_distances, _ = torch.min(distances, dim=1)
                    score = min_distances.max().item()  # Single scalar value

                    prediction = 1 if score > self.threshold else 0

                    all_scores.append(score)  # Use append for scalar
                    all_predictions.append(prediction)  # Use append for scalar
                    all_labels.extend(labels)  # Keep extend for labels (could be list)
                    all_paths.extend(paths)  # Keep extend for paths (could be list)
...
def main():
    ...
    train_dir = base_dir / "yolo" / "extracted_welds" / "train"
    test_dir = (
        base_dir / "yolo" / "extracted_welds" / "test"
    ...
    patchcore = WeldingPatchCore(
        train_data_dir=str(train_dir), test_data_dir=str(test_dir)
    )

    # Build memory bank from good welding patches
    patchcore.build_memory_bank(subsample_ratio=0.001)  # Very small memory bank
    ...
    if test_dir.exists():
        auc_score, predictions, labels = patchcore.evaluate(
            test_data_dir=str(test_dir), save_dir=str(results_dir)
        )
